...
void iosc_freq_init(void)
{
	...
	if (already_init_osc_freq == 0) {
		...
		if (!ht || !xt ||
		    ht > 0xffffffffU / (CCU_DCXO_FREQ / 1000)) {
			WRN("invalid IOSC calibration sample: dcxo=%d iosc=%d\n", ht, xt);
			return;
		}
		xf = ((CCU_DCXO_FREQ / 1000) * ht) / xt;
...
s32 ccu_init(void)
{
	...
	writel((readl(APBS1_CFG_REG) & (~APBS1_CLK_SRC_SEL_MASK)) | APBS1_CLK_SRC_SEL(4), APBS1_CFG_REG);
#endif
