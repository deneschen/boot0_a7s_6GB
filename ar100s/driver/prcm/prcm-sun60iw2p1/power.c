...
s32 ccu_24mhosc_disable(void)
{
	...
	/* enable rtc config clock */
	value = readl(RTC_CFG_REG);
	writel(value | (1 << 31), RTC_CFG_REG);
	/* power-off dcxo */
	value = 0x16aa;
	...
	value = readl(RTC_AUTO_CALI_CTRL);
	value &= (~(0x1 << 0));
	writel(value, RTC_AUTO_CALI_CTRL);

	value = ((readl(RTC_LOSC_CTRL) & 0xffff) | (0x16aa << 16));
	writel(value, RTC_LOSC_CTRL);
	writel(readl(RTC_LOSC_CTRL) & (~(0x1<<0)) & (~(0x1<<1)) & (~(0x1<<4)), RTC_LOSC_CTRL);
	return OK;
}
...
s32 ccu_24mhosc_enable(void)
{
	...
	writel(value, RTC_LOSC_CTRL);
	time_mdelay(2);
	while ((readl(RTC_LOSC_AUTO_STA) & 0x04) != 0)
		;
	time_mdelay(2);
	/* make sure the 32k clock from rc*/
	value = (value | (0x1 << 0));
	writel(value, RTC_LOSC_CTRL);
