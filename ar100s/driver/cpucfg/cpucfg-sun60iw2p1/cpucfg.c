...
s32 cpucfg_remove_little_endian_address(void *start, void *end)
{
	...
	for (i = little_endian_used - 1; i >= 0; i--) {
		if ((readl(CPUS_SPACE_START_REG(i)) >> 9 == start_addr >> 9) &&
		    (!end || readl(CPUS_SPACE_END_REG(i)) >> 9 == end_addr >> 9))
			break;
	}
	if (i < 0) {
		WRN("little-endian config not found\n");
		return -ENODEV;
	}

	INF("remove:%d\n", i);
	last = little_endian_used - 1;

	/* Keep active windows densely packed for the next allocation. */
	value = readl(CPUS_SPACE_LE_CTRL_REG);
	value &= ~((0x1 << i) | (0x1 << last));
	if (i != last) {
		writel(readl(CPUS_SPACE_START_REG(last)), CPUS_SPACE_START_REG(i));
		writel(readl(CPUS_SPACE_END_REG(last)), CPUS_SPACE_END_REG(i));
		value |= 0x1 << i;
	}
	writel(value, CPUS_SPACE_LE_CTRL_REG);

	/* Clean the now-unused final slot. */
	writel(0, CPUS_SPACE_START_REG(last));
	writel(0, CPUS_SPACE_END_REG(last));
...
static void cpucfg_counter_init(void)
{
	writel(CCU_SYS_CLK24M_FREQ, CNT_FREQID_REG);
}
